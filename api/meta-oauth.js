...
async function callback(request, response) {
  ...
  try {
    ...
    await serviceRequest("/rest/v1/meta_authorizations?on_conflict=user_id", { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify({ user_id: state.userId, external_user_id: identity.id, external_user_name: identity.name, encrypted_access_token: encryptToken(token.access_token), token_expires_at: expiresAt, status: "active", last_error: null, updated_at: new Date().toISOString() }) });
    response.setHeader("Content-Type", "text/html; charset=utf-8");
    return response.status(200).send(callbackPage({ ok: true, name: identity.name }));
  } catch (error) {
    console.error(error);
    response.setHeader("Content-Type", "text/html; charset=utf-8");
    return response.status(error.statusCode || 500).send(callbackPage({ ok: false, error: error.message || "Lỗi kết nối Meta." }));
  }
}

export default function handler(request, response) { return request.query?.route === "callback" ? callback(request, response) : start(request, response); }
